...
class DatabaseBackupManager:    
    def __init__(self, config, mongodb, logger):
      ...
      self.backup_db_prefix = self.config.get('MONGODB_BACKUP_DB', 'backup')
      self.max_backups = self.config.get('BACKUP_RETENTION', 15)
      self.max_retries = self.config.get('BACKUP_MAX_RETRIES', 5)
      self.retry_delay = self.config.get('BACKUP_RETRY_DELAY', 5)
    ...
    def _get_backup_databases(self) -> List[str]:
      ...
      return [db for db in self.mongo_client.list_database_names() if self.backup_db_prefix in db]
    ...
    def _cleanup_old_backups(self, backup_databases: List[str]) -> None:
      ...
      if len(valid_backups) > self.max_backups:
        valid_backups.sort(key=lambda x: x[1])  # Tsort by date
    ...
    def _copy_database(self, backup_db_name: str) -> bool:
      ...
      """
      try:
        backup_db = self.mongo_client[backup_db_name]
        collections = self.mongo_db.list_collection_names()
        self.logger.info(f'Backup {len(collections)} collections to {backup_db_name}')
        for collection_name in collections:
          self.logger.info(f'  Saving collection: {collection_name}')
          backup_db.drop_collection(collection_name)
          source_collection = self.mongo_db[collection_name]
    ...
    def create_backup(self, operation_name: str='modification') -> str:
      """ Create a backup with auto retry
        Args:
          operation_name: description of the operation to be done
        Returns:
          str: created backup database name or 'error'
      """
      backup_db_name = self._generate_backup_name()
      for attempt in range(self.max_retries):
        self.logger.info(
          f'Backup cretion before {operation_name}'
